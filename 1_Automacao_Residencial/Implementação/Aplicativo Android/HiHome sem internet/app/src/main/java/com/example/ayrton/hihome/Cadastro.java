...
public class Cadastro extends AppCompatActivity implements View.OnClickListener{
    ...
    private DatabaseReference fb_reference = FirebaseDatabase.getInstance().getReference();
            ...
            fbauth.createUserWithEmailAndPassword(loginTxt, senhaTxt).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                ...
                public void onComplete(@NonNull Task<AuthResult> task) {
                    if (task.isSuccessful()) {
                        ...
                        clienteBanco cliente = new clienteBanco(lampada, ac, portao);

                        fb_reference.child("usuarios").child(task.getResult().getUser().getUid()).setValue(cliente);
